...
# e.g. `./patch_bsd.py freebsd.reg freebsd.mem kernel 0xc0000000`
if len(sys.argv) < 5:
    ...
    if prot.check_exec() and va > kernel_base:
        ...
        for nullpad in re.finditer(b"(\x00+)", page):
            ...
            if len(nullpad.group()) > 0x200 and main_patch_size < len(nullpad.group()):
                ...
                print(len(sig))
                print(hex(koff))
                if koff != -1 and len(sig) != 0:
                    main_patch_vaddr, off = align(va+nullpad.start())
                    main_patch_off = koff+nullpad.start()+off
                    main_patch_size = len(nullpad.group())-off
